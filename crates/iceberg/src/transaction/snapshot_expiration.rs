...
//! Snapshot expiration for a commit.
//!
...
//! [`SnapshotExpirationPlan::collect_protected`] — but the metadata is the one the plan
...
pub(crate) fn expiration_updates(table: &Table) -> Result<Vec<TableUpdate>> {
    expiration_updates_at(table, chrono::Utc::now().timestamp_millis())
}

/// [`expiration_updates`] with the clock handed in.
///
...
/// clock is read, and it is the only caller outside tests.
fn expiration_updates_at(table: &Table, now_ms: i64) -> Result<Vec<TableUpdate>> {
    let metadata = table.metadata();
    let Some(policy) = SnapshotExpirationPolicy::resolve(metadata)? else {
        return Ok(Vec::new());
    };

    let plan = SnapshotExpirationPlan::new(metadata, &policy, now_ms);
...
/// Read one bound of the window: parse it when the table sets it, fall back to the
/// spec default when it does not, fail when it is set to something unparsable.
fn parse_policy_value<T: std::str::FromStr>(
    ...
    <T as std::str::FromStr>::Err: std::fmt::Display,
{
    properties.get(key).map_or(Ok(default), |raw| {
        raw.parse::<T>()
            .map_err(|e| invalid_policy(format!("Invalid value for {key}: {e}")))
    })
}
...
#[derive(Debug)]
struct SnapshotExpirationPolicy<'a> {
...
impl<'a> SnapshotExpirationPolicy<'a> {
    ...
    fn resolve(metadata: &'a TableMetadata) -> Result<Option<Self>> {
        ...
        let Some(raw_enabled) = properties.get(TableProperties::PROPERTY_HISTORY_EXPIRE_ENABLED)
        else {
            return Ok(None);
        };
        let enabled = raw_enabled.parse::<bool>().map_err(|e| {
        ...
        let min_snapshots_to_keep = parse_policy_value::<usize>(
            properties,
            TableProperties::PROPERTY_HISTORY_EXPIRE_MIN_SNAPSHOTS_TO_KEEP,
            TableProperties::PROPERTY_HISTORY_EXPIRE_MIN_SNAPSHOTS_TO_KEEP_DEFAULT,
        )?;
        ...
        let max_snapshot_age_ms = parse_policy_value::<i64>(
            properties,
            TableProperties::PROPERTY_HISTORY_EXPIRE_MAX_SNAPSHOT_AGE_MS,
            TableProperties::PROPERTY_HISTORY_EXPIRE_MAX_SNAPSHOT_AGE_MS_DEFAULT,
        )?;
        ...
        Ok(Some(SnapshotExpirationPolicy {
...
struct SnapshotExpirationPlan {
    ...
    /// [`SnapshotExpirationPlan::collect_protected`].
    preserve_carrier_missing: bool,
}

impl SnapshotExpirationPlan {
    ...
    fn new(
        ...
        policy: &SnapshotExpirationPolicy<'_>,
        now_ms: i64,
    ) -> SnapshotExpirationPlan {
...
mod tests {
    ...
    fn table_with(metadata: TableMetadata) -> Table {
        Table::builder()
            .metadata(metadata)
    ...
    fn updates_for(metadata: TableMetadata, now_ms: i64) -> Result<Vec<TableUpdate>> {
        expiration_updates_at(&table_with(metadata), now_ms)
    }
    ...
    fn missing_min_snapshots_to_keep_falls_back_to_the_spec_default() {
        let mut properties = enabled_properties(3, 0);
        properties.remove(TableProperties::PROPERTY_HISTORY_EXPIRE_MIN_SNAPSHOTS_TO_KEEP);
        let updates = updates_for(metadata_with_chain(5, properties), now_after_history()).unwrap();
        assert_eq!(
            expired_ids(&updates),
            vec![1, 2, 3, 4],
    ...
    fn missing_max_snapshot_age_falls_back_to_the_spec_default() {
        let mut properties = enabled_properties(3, 0);
        properties.remove(TableProperties::PROPERTY_HISTORY_EXPIRE_MAX_SNAPSHOT_AGE_MS);
...
mod commit_tests {
    ...
    async fn an_invalid_policy_fails_the_commit_without_reaching_the_catalog() {
        ...
        assert!(
            error
                .message()
                .contains(TableProperties::PROPERTY_HISTORY_EXPIRE_MIN_SNAPSHOTS_TO_KEEP),
